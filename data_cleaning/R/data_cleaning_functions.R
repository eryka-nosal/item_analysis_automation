...
# Prints group of cleaning statements - for counts so far
print.cleaning.output <- function(cleaning_output, v, step_name, num_responses_current, num_seq_current, num_users_current, num_items_current) {
  cleaning_output <- print.if.verbose(paste0("---------- Step: ", step_name, " ----------"), v, cleaning_output)
  cleaning_output <- print.if.verbose(paste0("Total responses at start:", "\t", num_responses_current), v, cleaning_output)
  cleaning_output <- print.if.verbose(paste0("Total activities at start:", "\t", num_seq_current), v, cleaning_output)
...
# Prints group of cleaning statements - for counts of what was removed
print.removal.output <- function(cleaning_output, v, step_name, num_responses_removed, num_seq_removed, num_users_removed, num_items_removed) {
  cleaning_output <- print.if.verbose(paste0("---------- Step: ", step_name, " ----------"), v, cleaning_output)
  cleaning_output <- print.if.verbose(paste0("Responses removed:", "\t","\t", num_responses_removed), v, cleaning_output)
  cleaning_output <- print.if.verbose(paste0("activities removed:", "\t","\t", num_seq_removed), v, cleaning_output)
  cleaning_output <- print.if.verbose(paste0("Users removed:", "\t","\t","\t", num_users_removed), v, cleaning_output)
  cleaning_output <- print.if.verbose(paste0("Unique content items removed:", "\t", num_items_removed), v, cleaning_output)
  return(cleaning_output)
}
...
describe.dupe.cor.ans <- function(df, use.content.item.name = FALSE) {
  if (use.content.item.name == TRUE) {
    ...
    outputdf <-
      df_to_summarize %>% group_by(content_item_name, raw_response) %>% summarise(total =
                                                                              length(content_item_name))
    outputdf
  } else {
    ## use.content.item.name is FALSE
    cadf <- df %>%
      ungroup() %>%
    ...
    if (length(unique(cadf$content_item_id)) < length(unique(df$content_item_id))) {
      ...
      cadf <- df %>%
        filter(scored_response == 1) %>%
        distinct(content_item_id, raw_response)
      names(cadf)[2] <- "correct_answer"
    }
    ...
    if (!is.null(df_to_summarize$timestamp_created)) {
      outputdf <- df_to_summarize %>%
        group_by(content_item_id, raw_response) %>%
        summarise(
...
get.item.cor.ans <- function(df, no.correct.answer = FALSE, use.content.item.name = FALSE) {
  if(use.content.item.name == TRUE) {
    if (no.correct.answer) {
      ...
      if (length(unique(cadf$content_item_name)) < dim(cadf)[1]) {
        ...
        print(describe.dupe.cor.ans(df, use.content.item.name = use.content.item.name))
      ...
      if (length(unique(cadf$content_item_name)) < length(unique(df[!is.na(df$correct_answer),]$content_item_name))) {
        ## making sure to degroup here, because otherwise dplyr will "helpfully" add back in the grouped columns like activity_id
        cadf <- df %>%
          dplyr::filter(scored_response == 1) %>%
          dplyr::ungroup() %>%
  ...
  } else { ## use.content.item.name is FALSE
    if (no.correct.answer) {
      cadf <- df %>%
        dplyr::filter(scored_response == 1) %>%
        dplyr::ungroup() %>%
...
removed.record.count <- function(df, thing.to.say = "activities removed: " , include.item.count = FALSE) {
  ...
  num_seq_new <- length(unique(df$activity_id_hist))
  ...
  assign("cleaning_info",
         value = print.if.verbose(paste0(thing.to.say, ## defaults to activities removed
...
remove.repeat.questions <- function(df, remove = FALSE, add.col = FALSE) {
  df <- df %>%
    dplyr::ungroup() %>%
    # order by the times the person saw the question - dates on activities
